...
C	  PMU = 2.0*PK/PLAM-1.0 
	  PAR(1) =2.0*PAR(8)/PAR(4)-1.0
	  PMU =2.0*PAR(8)/PAR(4)-1.0
...
C
        PAR(1) = 0.106535	!PMU= C and MU for each eq.
		PAR(2) = 0.35	!A
		PAR(3) = 0.0	!phi = PH
		PAR(8) = 1.25	!k = PK
		PAR(9) = 0.0163	!c=PC
...
C	  U(x)=6a/(2(1+a)+Sqrt[2(2-a)(1-2a)] Cosh[Sqrt[a] x])
      S=(T-0.5)*PAR(11)
	  H1 = SQRT(2.0*(2.0-PAR(2))*(1.0-2.0*PAR(2)))
	  H1 = H1*COSH(SQRT(PAR(2))*S)+2.0*(1.0+PAR(2))
      U(1) = EP2*6.0*PAR(2)/H1+EP1
C
      H2 = -6.0*SQRT(2.0*(2.0-PAR(2))*(1.0-2.0*PAR(2)))
	  H2 = H2*PAR(2)*SQRT(PAR(2))*SINH(SQRT(PAR(2))*S)
	  U(2) = EP2*H2/H1/H1
C
      RETURN
      END
